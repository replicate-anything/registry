...
Study metadata lives in `studies/<folder>.yml` stub files. The catalog and machine-readable index are generated by rendering `index.qmd`, which writes `index.csv` (used by `replicateEverything` and the Shiny app) and `index.html`.
